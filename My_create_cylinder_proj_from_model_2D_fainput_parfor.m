...
function [cylinderProjAr, cylinerIndsAr] = My_create_cylinder_proj_from_model_2D_fainput_parfor(model, atomtype, currAtomInd, projAngles, Heights, Bfactors, totVolSize,Res, CropHalfWidth, cylinderRad, cylinderAtomBufferRad, fixedfa)
...
%   Vol = My_create_volProj_from_model_exact_HB_fixedfa_2D(inputModel*Res, inputAtomType, Heights, Bfactors, [cylinderRad*2+1 cylinderRad*2+1 totVolSize(3)] + cylinderRad*2, Res, CropHalfWidth, make_fixedfa(cylinderRad*2+1 + cylinderRad*2, Res));
    Vol = My_create_volProj_from_model_exact_HB_fixedfa_2D_fainput(inputModel*Res, inputAtomType, Heights, Bfactors, [cylinderRad*2+1 cylinderRad*2+1 totVolSize(3)] + cylinderRad*2, Res, CropHalfWidth, fixedfa);

  Vol = My_stripzero_ajp(Vol,[cylinderRad*2+1 cylinderRad*2+1]);
